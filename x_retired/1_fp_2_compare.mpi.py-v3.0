...
def main( reference, queue, rank_fp, mpi_np, output ):
  ...
  Rfp = [x for x in tqdm( mpi.imap_unordered(
                              GenerateFPs, list(zip(Ref.mol.tolist(), RfNm))),
                              total=len(list(RfNm)) ) ]
  Qfp = [x for x in tqdm( mpi.imap_unordered(
                              GenerateFPs, list(zip(Que.mol.tolist(), QuNm))), 
                              total=len(list(QuNm)) ) ]
...
#    pRef = FPFunctions()     # Calculate reference fp only once
    ...
    Data = [x for x in tqdm(mpi.imap_unordered(pLib, Qfp),total=len(Qfp))]
...
############
  def CompareFPs(self, Item):
    ...
    Data  = [ que_id, self.ref_id, Tc[0], Tc[1], Tc[2], 
              ( Tc[0]*dl_r + Tc[1]*ec_r + Tc[2]*ms_r )/(dl_r + ec_r + ms_r),
              que_sm ]
    return Data
